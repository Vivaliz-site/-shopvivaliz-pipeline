...
A failed validation, check, conflict, runner problem, permission issue, or external dependency blocks task completion but does not authorize abandoning the PR. Diagnose, repair, revalidate, and repeat until merge. If an external blocker temporarily prevents progress, the task remains active/incomplete and must resume when the blocker can be addressed.
